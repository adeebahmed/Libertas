...
def sync_ofx_connection(
    ...
    try:
        ...
        return _fail(f"OFX connection config missing required field: {exc}")
    ...
    for raw in raw_txs:
        try:
            ...
            failed += 1
            logger.warning("OFX sync: skipping transaction due to error: %s", exc)

    conn.last_sync_at = datetime.now(timezone.utc)
    conn.last_error = None
    conn.status = "active"
    run.status = "success"
    run.details = {"imported": imported, "skipped": skipped, "failed": failed}
    run.finished_at = datetime.now(timezone.utc)
    db.flush()

    logger.info("OFX sync connection=%s imported=%s skipped=%s failed=%s", conn.id, imported, skipped, failed)
    return {
        "status": "success",
        "connection_id": conn.id,
        "imported": imported,
