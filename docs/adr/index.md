# Architecture Decision Records

This index tracks major product and engineering decisions for Libertas.

## Current ADRs

- [ADR-001 — Finance Dashboard Design](/adr/001-finance-dashboard-design)
- [ADR-002 — Data Ingestion Strategy: Multi-Source Support](/adr/002-data-ingestion-strategy)
- [ADR-002 — Taxes Page](/adr/002-taxes-page)
- [ADR-003 — News Feed Integration](/adr/003-news-feed)
- [ADR-004 — User Profile and AI-Powered Guidance](/adr/004-user-profile-and-ai-guidance)
- [ADR-005 — Versioned Backups and Rollback](/adr/005-versioned-backups)
- [ADR-006 — Tier-1 Net Worth Dashboard Completion](/adr/006-tier1-dashboard-completion)
- [ADR-009 — Optional Plaid + Sheets Integration with Canonical Multi-Source Ingest](/adr/009-optional-plaid-and-sheets-integration)
- [ADR-010 — At-Rest Encryption](/adr/010-at-rest-encryption)
- [ADR-011 — Overview Collapsed Market Tape](/adr/011-dashboard-market-tape)

## Notes

- ADR numbering includes historical duplicates (`ADR-002`) from earlier phases.
- New significant subsystems should ship with a new sequential ADR in `docs/adr/`.
